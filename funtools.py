...
def insert_for_against():
    ...
    test = np.zeros((team_num,  extra_col), dtype=np.int64)
    for x in range(3, (rnds*col_delta), col_delta):
...
def make_header():
    ...
    header = ["Team No"]
    rnd_count = 1
    for x in range(1, rnds*col_delta, col_delta):
...
def excel_stuff():
    ...
    bd = Side(style='thin', color='000000')
    bd1 = Side(style='medium', color='000000')
    my_agnst_sty.border=Border(left=bd, top=bd, right=bd, bottom=bd)
    my_agnst_sty.alignment  = Alignment(horizontal='center', vertical='center')
    my_agnst_sty.fill = PatternFill("solid", fgColor='00FFCCDD')

    my_ends_sty = NamedStyle(name="my_ends_sty")
    my_ends_sty.font=Font(bold=False, size=11)
    bd = Side(style='thin', color='000000')
    bd1 = Side(style='medium', color='000000')
    my_ends_sty.border=Border(left=bd, top=bd, right=bd1, bottom=bd)
    my_ends_sty.alignment  = Alignment(horizontal='center', vertical='center')
    my_ends_sty.fill = PatternFill("solid", fgColor='0099BBFF')
    ...
    for r in range(3, (rnds*col_delta), col_delta):
        my_ary_for.append(full_alpha[r])

    for p in range(4, (rnds*col_delta)+1, col_delta):
        my_ary_against.append(full_alpha[p])

    for a in range(0, (rnds * col_delta)+6):  # excel alhpa for column names for header
        my_ary_header.append((full_alpha[a]))

    for d in range(3, (rnds*col_delta), col_delta):   # for calculating the points
    ...
    for w in range(2, team_num + 2): # sum the for scores
        ...
        sheet[f'{full_alpha[(rnds*col_delta)+2]}{w}'] = f'= SUM({final_array})'
        sheet[f'{full_alpha[(rnds*col_delta)+2]}{w}'].alignment  = Alignment(horizontal='center', vertical='center')
    ...
    for w in range(2, team_num + 2): # sum the against scores
        ...
        sheet[f'{full_alpha[(rnds*col_delta)+3]}{w}'] = f'= SUM({final_array_a})'
        sheet[f'{full_alpha[(rnds*col_delta)+3]}{w}'].alignment  = Alignment(horizontal='center', vertical='center')

# this is the area used to make the points formula ---------------------------------------------------#

    for w in range(2, team_num + 2):
        points_array = []
        for z in range(3, (rnds*col_delta)  , col_delta):
            if z != (rnds*col_delta)-1 and use_end_wins ==0:
                print(z , '-',(rnds*col_delta)-1)
                for_e = full_alpha[z] + str(w)
                agst_e = full_alpha[z+1] + str(w)
                points_array.append('IF(' + for_e + '>' + agst_e + ','+ str(score_sys[0])+',IF(AND(' + for_e + '='+agst_e + ','
                                    + for_e + '<>0),'+ str(score_sys[1])+',0))+')
            elif z != (rnds*col_delta)-2 and use_end_wins ==1:
               # print(z , '-',(rnds*col_delta)-2)
                for_e = full_alpha[z] + str(w)
                agst_e = full_alpha[z+1] + str(w)
                ends_e = full_alpha[z+2] + str(w)
                points_array.append('IF(' + for_e + '>' + agst_e + ','+ str(score_sys[0]) + ',IF(AND(' + for_e + '='+agst_e + ','
                                    + for_e + '<>0),'+ str(score_sys[1]) + ',0))+'+ ends_e +'+')

            elif z == (rnds*col_delta)-1 and use_end_wins ==0:
                for_e = full_alpha[z] + str(w)
                agst_e = full_alpha[z+1] + str(w)
                points_array.append('IF(' + for_e + '>' + agst_e + ','+ str(score_sys[0])+',IF(AND(' + for_e + '='+agst_e + ','
                                    + for_e + '<>0),'+ str(score_sys[1])+',0))')
        ...
        # print(final_array_p)
        sheet[f'{full_alpha[(rnds*col_delta)+1]}{w}'] = f'={final_array_p}'  # SUM game results
        sheet[f'{full_alpha[(rnds*col_delta)+4]}{w}'] = f'=IFERROR({full_alpha[(rnds*col_delta)+2]}{w}/{full_alpha[(rnds*col_delta)+3]}{w}' \
                                                f',0)'  # Percentage
        sheet[f'{full_alpha[(rnds*col_delta)+5]}{w}'] = f'=RANK({full_alpha[(rnds*col_delta)+1]}{w},${full_alpha[(rnds*col_delta)+1]}$2:${full_alpha[(rnds*col_delta)+1]}${team_num+1})' \
                                               f'+ COUNTIFS(${full_alpha[(rnds*col_delta)+1]}$2:${full_alpha[(rnds*col_delta)+1]}${team_num+1}' \
                                               f',${full_alpha[(rnds*col_delta)+1]}{w},${full_alpha[(rnds*col_delta)+4]}$2:${full_alpha[(rnds*col_delta)+4]}${team_num+1}' \
                                               f',">"&${full_alpha[(rnds*col_delta)+4]}{w})' # this calcualtes the standings
        sheet[f'{full_alpha[(rnds*col_delta)+1]}{w}'].alignment  = Alignment(horizontal='center', vertical='center')
        sheet[f'{full_alpha[(rnds*col_delta)+4]}{w}'].alignment  = Alignment(horizontal='center', vertical='center')

    for t in range(0, (rnds * col_delta)+6):
    ...
    for x in range (2, (rnds * col_delta) + 1,col_delta): # format each Vs info in excel
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
            sheet[full_alpha[x]+str(y)].style = my_Vs_sty

    for x in range (1, (rnds * col_delta) + 1,col_delta): # format each Rink info in excel
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
            sheet[full_alpha[x]+str(y)].style = my_rink_sty

    for x in range (3, rnds * col_delta,col_delta): # format each games For info in excel
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
    ...
    for x in range (4, (rnds * col_delta) + 1,col_delta): # format each games Against info in excel
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
    ...
    if use_end_wins == 1:
        for x in range (5, (rnds * col_delta) + 2,col_delta): # format each games ends won info in excel

            for y in range (2, team_num+ 2):
                #print(y)
                sheet[full_alpha[x]+str(y)].style = my_ends_sty
                sheet[full_alpha[x]+str(y)].protection = Protection(locked=False)


    for x in range ((rnds * col_delta) + 2, (rnds * col_delta) + 5,1): # format Tot For/Tot Against/Percentage info in excel
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
            sheet[full_alpha[x]+str(y)].style = my_results_sty

    for x in range ((rnds * col_delta)+ 4, (rnds * col_delta)+ 5,1): # format Percentage column
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
            sheet[full_alpha[x]+str(y)].number_format = '0.00%'

    for x in range ((rnds * col_delta)+ 5, (rnds * col_delta)+ 6,1): # format standings and Points column
        # print(full_alpha[x])
        for y in range (2, team_num+ 2):
            #print(y)
    ...
    for x in range (1, (rnds * col_delta)+ 6,1): # format boareder on last row
        # print(full_alpha[x])
    ...
    sheet.conditional_formatting.add(f'{full_alpha[(rnds*col_delta)+1]}{2}:{full_alpha[(rnds*col_delta)+1]}{team_num+1}',
                                     ColorScaleRule(start_type='min',start_color='00FFFFFF', end_type='max',
                                                    end_color='00FFD500'))
    # ------------------------------- Set the conditional formatting for the standings column ------------ #
    sheet.conditional_formatting.add(f'{full_alpha[(rnds*col_delta)+5]}{2}:{full_alpha[(rnds*col_delta)+5]}{team_num+1}',
...
def make_my_draw(num_of_rnds=4, num_of_teams=32, draw_random=0, count_end_wins=0, alt_points=0):
    global rnds
    global team_num
    global grid
    global rinks
    ...
    global a
    global use_end_wins
    global use_alt_points
    global score_sys
    global col_delta
    global extra_col

    use_end_wins = count_end_wins # variable used to indicate if ends won should count towards points total, 1 point per end win
    use_alt_points = alt_points   # variable used to select alternative win/draw points values, 10/5 or 10/4
